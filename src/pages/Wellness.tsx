...
const Wellness = () => {
  ...
  useEffect(() => {
    const loadPosts = async () => {
      try {
        const fetchedPosts = await postsApi.fetchByCategory("Wellness");
        setPosts(fetchedPosts);
      } catch (error) {
        console.error('Failed to load wellness posts:', error);
      } finally {
        setIsLoading(false);
      }
    };
  ...
  const wellnessArticles = posts.map(post => ({
    id: post.id,
    title: post.title,
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {isLoading ? (
                ...
                <div className="col-span-full text-center py-12">
                  <p className="text-muted-foreground">No wellness articles yet.</p>
                </div>
              )}
            </div>
          </section>
